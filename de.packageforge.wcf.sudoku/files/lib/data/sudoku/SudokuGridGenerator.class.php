...
use wcf\data\sudoku\propagator\JumpPropagator;
use wcf\data\sudoku\propagator\LRTBPropagator;
use wcf\data\sudoku\propagator\RandomPropagator;
use wcf\data\sudoku\propagator\SPropagator;
...
class SudokuGridGenerator extends SingletonFactory {
	 ...
	 */
	public function generateSudokuGrid($difficulty = self::DIFFICULTY_EASY) {
		$grid = null;
		...
		switch ($difficulty) {
			case self::DIFFICULTY_EXTREMELY_EASY:
				...
				$propagator = new RandomPropagator();
				break;
		...
		do {
				...
				if ($cellsInRow <= $lowerRowColumnRestriction || $cellsInColumn <= $lowerRowColumnRestriction) {
					...
					foreach ($possibleValues as $value) {
						if ($value != $cell->getValue()) {
							...
							if ($testGrid !== null && !$testGrid->compareTo($terminalPattern)) {
								$unique = false;
								$cell->toggleCanBeDug();
								break;
							}
			...
			$propagator->propagate($row, $column);
		}
